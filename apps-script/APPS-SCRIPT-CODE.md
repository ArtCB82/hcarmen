...
const HEADERS_USUARIOS = [
  'ID', 'Nombre', 'Rol', 'PinHash', 'Salt', 'Color', 'Activo', 'CreatedAt'
];

const SESSION_TTL_MS = 60 * 60 * 1000;
const DEFAULT_USUARIOS = [
  { id: 'usr_admin', nombre: 'Arturo', rol: 'admin', pin: '1982', color: '#2d6a4f', activo: true },
  { id: 'usr_arturo', nombre: 'Arturo H', rol: 'user', pin: '1234', color: '#1d4ed8', activo: true },
  { id: 'usr_arturop', nombre: 'Arturo P', rol: 'user', pin: '2081', color: '#e07a2f', activo: true },
  { id: 'usr_carmen', nombre: 'Carmen', rol: 'user', pin: '2081', color: '#c1440e', activo: true },
  { id: 'usr_alvaro', nombre: 'Alvaro', rol: 'user', pin: '0000', color: '#7c3aed', activo: true },
  { id: 'usr_sonia', nombre: 'Sonia', rol: 'user', pin: '0000', color: '#0891b2', activo: true }
];
...
function doGet(e) {
  ...
  try {
    ...
    switch (params.action) {
      case 'getFacturas': { const auth = requireAuth({ authToken: params.authToken }); result = auth.error ? auth : getFacturas(); break; }
      case 'getVentas': { const auth = requireAuth({ authToken: params.authToken }); result = auth.error ? auth : getVentas(); break; }
      case 'getGastos': { const auth = requireAuth({ authToken: params.authToken }); result = auth.error ? auth : getGastos(); break; }
      case 'getProveedores': { const auth = requireAuth({ authToken: params.authToken }); result = auth.error ? auth : getProveedores(); break; }
      case 'getAll': {
        const auth = requireAuth({ authToken: params.authToken });
      ...
      case 'testApi': { const auth = requireAuth({ authToken: params.authToken }, 'admin'); result = auth.error ? auth : testApiClaude(); break; }
      case 'getApiUsage': { const auth = requireAuth({ authToken: params.authToken }, 'admin'); result = auth.error ? auth : getApiUsage(params.mes); break; }
...
function procesarAccion(data) {
  try {
    let result;
    switch (data.action) {
      case 'login': result = loginUsuario(data.userId, data.pin); break;
      case 'getAll': {
        const auth = requireAuth(data);
        result = auth.error ? auth : {
          facturas: getFacturas(),
      ...
      case 'addFactura': { const auth = requireAuth(data); result = auth.error ? auth : addFactura(data.factura); break; }
      case 'addVenta': { const auth = requireAuth(data); result = auth.error ? auth : addVenta(data.venta); break; }
      case 'addGasto': { const auth = requireAuth(data); result = auth.error ? auth : addGasto(data.gasto); break; }
      case 'deleteRegistro': { const auth = requireAuth(data, 'admin'); result = auth.error ? auth : deleteRegistro(data.tipo, data.syncId); break; }
      case 'addLog': { const auth = requireAuth(data); result = auth.error ? auth : addLog(data.log); break; }
      case 'saveProveedor': { const auth = requireAuth(data); result = auth.error ? auth : saveProveedor(data.proveedor, data.isEdit); break; }
      case 'deleteProveedor': { const auth = requireAuth(data, 'admin'); result = auth.error ? auth : deleteProveedor(data.id); break; }
      case 'mergeDuplicateProveedores': { const auth = requireAuth(data, 'admin'); result = auth.error ? auth : mergeDuplicateProveedores(); break; }
      case 'testApi': { const auth = requireAuth(data, 'admin'); result = auth.error ? auth : testApiClaude(); break; }
      case 'getApiUsage': { const auth = requireAuth(data, 'admin'); result = auth.error ? auth : getApiUsage(data.mes); break; }
...
function getUsuariosSheet() {
  const sheet = getOrCreateSheet('Usuarios', HEADERS_USUARIOS);
  if (sheet.getLastRow() <= 1) {
    DEFAULT_USUARIOS.forEach(function(u) {
...
function getUsuariosAuth() {
  ...
  return data.slice(1).map(function(r) {
    return {
      ...
      salt: r[4] || '',
      color: r[5] || '#2d6a4f',
      activo: r[6] !== false && String(r[6]).toLowerCase() !== 'false'
    };
  }).filter(function(u) { return u.id && u.nombre; });
}

function loginUsuario(userId, pin) {
  const usuario = getUsuariosAuth().find(function(u) {
    return u.id === userId && u.activo;
  });
  if (!usuario || !pin || hashPin(pin, usuario.salt) !== usuario.pinHash) {
    return { success: false, error: 'INVALID_LOGIN' };
  }

  const token = Utilities.getUuid() + Utilities.getUuid().replace(/-/g, '');
  const expira = Date.now() + SESSION_TTL_MS;
  const session = JSON.stringify({ userId: usuario.id, expira: expira });
  CacheService.getScriptCache().put('session_' + token, session, Math.floor(SESSION_TTL_MS / 1000));
...
function getSessionUsuario(token) {
  ...
  if (!session.expira || session.expira <= Date.now()) {
    PropertiesService.getScriptProperties().deleteProperty(key);
    return null;
  }

  const usuario = getUsuariosAuth().find(function(u) {
    return u.id === session.userId && u.activo;
  });
  return usuario ? usuarioPublico(usuario) : null;
}

function requireAuth(data, requiredRole) {
  const usuario = getSessionUsuario(data && data.authToken);
  if (!usuario) return { error: 'AUTH_REQUIRED' };
  if (requiredRole === 'admin' && usuario.rol !== 'admin') return { error: 'FORBIDDEN' };
